...
const (
	// WildcardSign is the sign for a wildcard route
	WildcardSign = "*"

	// ParamSign is the sign for a parameter
	ParamSign = ":"

	// OptionalSign is the sign for an optional parameter
	OptionalSign = "?"
)
...
func (r *Router) Add(method, path string, handlers ...Handler) {
	...
	for _, part := range parts {
		if strings.HasPrefix(part, ParamSign) {
			route.ParamNames = append(route.ParamNames, strings.TrimPrefix(part, ParamSign))
		}
	}
...
func (r *Route) match(path string) (bool, map[string]string) {
	...
	for i, part := range routeParts {
		if strings.HasPrefix(part, ParamSign) {
			paramName := strings.TrimPrefix(part, ParamSign)
			params[paramName] = parts[i]
		} else if part == WildcardSign {
			return true, params
