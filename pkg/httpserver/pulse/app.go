...
const (
	...
	DefaultAppName = "Nethttp"

	// DefaultNetwork is the default network
	DefaultNetwork = "tcp"
)

func New(config ...Config) *Nethttp {
	app := &Nethttp{
		config: &Config{},
		server: &http.Server{},
		Router: NewRouter(),
...
func (p *Nethttp) Run(address string) {
	// setup handler
...
func (p *Nethttp) startupMessage(addr string) string {
	myFigure := figure.NewFigure("Nethttp Server", "", true)
	myFigure.Print()
